...
#+BEGIN_SRC emacs-lisp
  (defun init-computer-os ()
    (cond ((equal (system-name) "secunit") 'nix)
	  ...
	  ((equal (system-name) "MacBookPro") 'mac)
...
#+BEGIN_SRC emacs-lisp
  (when (equal (init-computer-context) 'work)
    ...
    (message "set shell-file-name to /bin/bash"))
#+END_SRC
...
[2025-10-21 Tue] Log asynchronous native compilation warnings without popping up the *Warnings* buffer:
...
Custom key binding to jump to column 80
...
[2025-10-24 Fri] Minimize auto-scrolling. Keeps things in other windows from drastically changing when Avy is invoked.
...
#+BEGIN_SRC emacs-lisp
  (use-package column-enforce-mode
    :after (prog-mode yaml-ts-mode enh-ruby-mode)
    :hook (prog-mode yaml-ts-mode enh-ruby-mode)
    :diminish
    :config
...
** TODO Base Mac key settings on system-type == darwin instead of system-name
:LOGBOOK:
- State "TODO"       from              [2025-11-13 Thu 15:29]
:END:
